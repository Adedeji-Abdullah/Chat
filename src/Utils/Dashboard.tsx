...
const Dashboard = () => {
  ...
  const fetchUsers = async (searchText: string = '') => {
    ...
    setError('')
    try {
      const response = await fetch(`http://localhost:3005/users${searchText ? `?search=${encodeURIComponent(searchText)}` : ''}`)
  ...
  const fetchConversation = async (peerEmail: string) => {
    ...
    try {
      const res = await fetch(`http://localhost:3005/messages?user=${encodeURIComponent(currentUserEmail)}&peer=${encodeURIComponent(peerEmail)}`)
